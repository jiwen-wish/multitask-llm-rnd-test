...
scrape_file = os.path.join(dirname, "..", "..", "data", "query", "top_used_wish_queries_offshore_scraped_googlesearch.json")
df_scrape = pd.read_json(scrape_file, lines=True)
train_path, val_path, test_path = [os.path.join(dirname, "..", "..", "data", "query", 
    f"top_used_wish_queries_offshore_scraped_googlesearch_{mode}.json") for mode in [
        "Train", "Val", "Test"]]
#%%
df_scrape_clean = df_scrape[(df_scrape.title.apply(lambda x: len(x) > 0))].drop_duplicates(
    ["query", "title"])
df_scrape_clean["title"] = df_scrape_clean["title"].apply(
    lambda x: " ".join(x.split(" ")[:-1]) if x.endswith("...") else x
).apply(
    lambda x: x.replace('...', '') if x.endswith("...") else x
)
df_scrape_clean = df_scrape_clean[['query', 'title']]
# %%
query_list = sorted(list(set(df_scrape_clean['query'])))
np.random.seed(42)
np.random.shuffle(query_list)

train_query = set(query_list[:int(len(query_list) * .95)])
val_query = set(query_list[int(len(query_list) * .95):int(len(query_list) * .975)])
test_query = set(query_list[int(len(query_list) * .975):])
